...
// Missio IA logo path (for headers)
const MISSIO_LOGO_PATH = path.join(__dirname, '.claude/skills/docx/logo-missio.png');
...
new Paragraph({
  ...
  children: [
    ...
    new TextRun({ text: "Colored", color: "FF0000", size: 28, font: "Verdana" }), // Verdana default
    ...
    new SymbolRun({ char: "00A9", font: "Verdana" })   // Copyright © - Verdana for symbols
  ]
})
```

## Styles & Professional Formatting (Missio IA Brand)

```javascript
// MISSIO IA STANDARD TEMPLATE - Verdana + Logo header
const fs = require('fs');
const path = require('path');

// Logo path - adjust based on your working directory
const logoPath = '/Users/luisalarcon/Documents/Pruebas/Claude Code/general-assistant/.claude/skills/docx/logo-missio.png';

const doc = new Document({
  styles: {
    default: { document: { run: { font: "Verdana", size: 22 } } }, // 11pt Verdana default
    paragraphStyles: [
      // Document title style - Verdana Bold
      { id: "Title", name: "Title", basedOn: "Normal",
        run: { size: 48, bold: true, color: "000000", font: "Verdana" },
        paragraph: { spacing: { before: 240, after: 120 }, alignment: AlignmentType.CENTER } },
      // Heading styles - Verdana Bold
      { id: "Heading1", name: "Heading 1", basedOn: "Normal", next: "Normal", quickFormat: true,
        run: { size: 32, bold: true, color: "000000", font: "Verdana" }, // 16pt
        paragraph: { spacing: { before: 240, after: 240 }, outlineLevel: 0 } },
      { id: "Heading2", name: "Heading 2", basedOn: "Normal", next: "Normal", quickFormat: true,
        run: { size: 26, bold: true, color: "000000", font: "Verdana" }, // 13pt
        paragraph: { spacing: { before: 180, after: 180 }, outlineLevel: 1 } },
      { id: "Heading3", name: "Heading 3", basedOn: "Normal", next: "Normal", quickFormat: true,
        run: { size: 24, bold: true, color: "333333", font: "Verdana" }, // 12pt
  ...
  sections: [{
      ...
      default: new Header({
        children: [new Paragraph({
          ...
          children: [new ImageRun({
            ...
            transformation: { width: 180, height: 43 }, // Proportional: 2818x672 → 180x43
            altText: { title: "Missio IA", description: "Logo Missio IA", name: "logo-missio" }
          })]
        })]
      })
    },
    children: [
      ...
      new Paragraph({ heading: HeadingLevel.HEADING_2, children: [new TextRun("Heading 2")] }),
      new Paragraph({ children: [new TextRun("Normal body text in Verdana 11pt.")] })
    ]
  }]
});
```

**Missio IA Brand Guidelines:**
- **Font:** Verdana (universal, clean, professional)
- **Header:** Logo Missio IA aligned right
- **Title:** 24pt Verdana Bold, centered
- **Heading 1:** 16pt Verdana Bold
- **Heading 2:** 13pt Verdana Bold
- **Body:** 11pt Verdana Regular

**Alternative Font Combinations:**
- **Verdana (Headers) + Verdana (Body)** - Missio IA standard, universally supported
...
- **Set a default font** using `styles.default.document.run.font` - Verdana is universally supported
- **Always include Missio IA logo** in the header aligned right
...
- **ALWAYS use Missio IA template** with Verdana font and logo header for brand consistency
- **ALWAYS set a default font** using `styles.default.document.run.font` - Verdana recommended
- **ALWAYS include Missio IA logo** in header aligned right
